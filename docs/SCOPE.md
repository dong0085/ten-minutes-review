...
- **Length is a time budget that scales with the bank, capped at 20 standard questions.** *(my call on the formula: `min(20, max(8, floor(bank_size / 8)))` — a 60-question bank gives 8, a 100-question bank gives 12, and the cap holds from 160 up.)* A vocabulary question is quick to answer and counts as half a standard question, so a vocabulary-heavy day holds more questions. Each classroom's Settings carries a five-step quiz length (`<<` `<` `*` `>` `>>`) that multiplies the budget by 0.5, 0.75, 1, 1.5, or 2, never going below 5. The budget alone sets the length; there is no separate question cap.
