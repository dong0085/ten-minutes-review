...
export function TodayQuizAction({
  ...
  nowMs: number;
  autoStart?: boolean;
  initialJob: { id: string; status: JobStatus; requestedAt: string } | null;
  resetLocal: string;
  resetUtc: string;
  ...
  // The compose job this card started (or found in flight); its result names the new quiz.
  ...
  const fetchJob = useCallback(
    async (id: string): Promise<JobResponse | null> => {
      const response = await fetch(`/api/classrooms/${classroomId}/quizzes/jobs/${id}`);
      if (response.status === 404) {
        return { status: "failed", quizId: null };
      }
      if (!response.ok) {
        return null;
      }
  ...
  useEffect(() => {
    if (phase !== "composing" || !jobId) {
      return;
    }
    const interval = setInterval(() => {
      setElapsed((current) => current + POLL_MS / 1000);
      void fetchJob(jobId).then((data) => {
        if (!data) {
          return;
        }
        if (data.status === "done" && data.quizId) {
          setReadyQuizId(data.quizId);
          setPhase("ready");
          router.refresh();
          return;
        }
        ...
        if (data.status === "running" && step === "pending") {
  ...
  }, [phase, jobId, fetchJob, router, step]);
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      const data = (await response.json()) as { jobId?: string | null };
      if (!data.jobId) {
        setPhase("failed");
        return;
      }
  ...
  const cancel = useCallback(async () => {
    try {
      ...
      if (data.outcome === "too_late") {
        // The quiz is already being saved; polling the job will open it.
        return;
      }
      setPhase("stopped");
      router.refresh();
          ...
          {activeQuizId ? (
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <Button asChild>
                  <Link to={`/classrooms/${classroomId}/quizzes/${activeQuizId}/take`}>
                    {t("takeToday")}
                    <ArrowRight />
                  </Link>
                </Button>
          ...
          {composing && !timedOut ? (
            ...
            <>
              ...
              <DialogFooter>
                ...
                <Button asChild>
                  <Link to={`/classrooms/${classroomId}/quizzes/${readyQuizId}/take`}>
                    {t("take")}
                  </Link>
                </Button>
              </DialogFooter>
